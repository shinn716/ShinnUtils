    ...
    private void Update()
    {
        switch (nowState)
        {
            ...
            case State.Playing:
                ...
                if (m_ConvertImage != null)
                {
                    s = Sprite.Create((Texture2D)m_rawImage.texture, new Rect(0, 0, m_rawImage.texture.width, m_rawImage.texture.height), Vector2.zero);
                    m_ConvertImage.sprite = s;
                }
